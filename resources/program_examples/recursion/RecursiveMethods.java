 ...
 * @author Koffman and Wolfgang (methods labeled as chapter examples)
 * @author mvail (driver and some additional methods)
 */
public class RecursiveMethods {
	...
	public static void main(String[] args) {
		...
		System.out.println(listLength(list, 0));
		
		//the length of a String?
		String sleeeep = "This class puts me to sleeeeeeeeee...";
		System.out.println(strLength(sleeeep));
		System.out.println(sleeeep.length());
		
		//print each char from a String, one per line?
		printChars(sleeeep);
		
		//print chars from a String in reverse?
		System.out.println(reverseChars(sleeeep));
		...
		int[] array = { 2, 5, 3, 1 };
		int[] array2 = { 4, 3, 1, 5 };
		System.out.println(minFromArray(array, 0));
		System.out.println(minFromArray(array2, 0));
		
		//linear search? (aka "contains")
		System.out.println("is 3 in the array? " + contains(array, 3, 0));
		System.out.println("is 7 in the array? " + contains(array, 7, 0));
		
		//fibonacci
		System.out.println("7th fibonacci number: " + fibonacci(6));
		System.out.println("8th fibonacci number: " + fibonacciStart(8));
	}
	
	private static int listLength(ArrayList<Integer> list, int startingIndex) {
		if (startingIndex == list.size()) {
			return 0;
		} else {
			return 1 + listLength(list, startingIndex+1);
		}
	}
	...
	private static int minFromArray(int[] array, int startingIdx) {
		if (startingIdx == array.length-1) {
			return array[startingIdx];
		} else {
			return Math.min(array[startingIdx], minFromArray(array, startingIdx+1));
		}
	}
	
	private static boolean contains(int[] array, int target, int startingIdx) {
		if (startingIdx == array.length) {
			return false;
		} else {
			return (array[startingIdx] == target) || contains(array, target, startingIdx+1); //short circuiting!
		}
	}
